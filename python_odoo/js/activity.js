...
var BasicActivity = AbstractField.extend({
    ...
    */
    ...
    $form.find('#activity_feedback').val(self._draftFeedback[activityID]);
        ...
        // Ambil feedback
        var feedback = $form.find('#activity_feedback').val();

        // Ambil input file dari form
        var fileInput = $form.find('#activity_feedback_attachment')[0];
        var files = fileInput ? fileInput.files : [];

        if (!files.length) {
            console.log("No files uploaded. Proceeding with activity completion...");
            self._markActivityDone({ activityID: activityID, feedback: feedback, attachmentIds: [] });
            return;
        }

        console.log("Uploading files before completing activity...", files);
        ...
        Array.from(files).forEach(function (file) {
            ...
            reader.onload = function (e) {
                ...
                // Kirim file sebagai attachment ke Odoo
                self._rpc({
                    model: 'ir.attachment',
                    method: 'create',
                    args: [{
                        ...
                        datas: base64Data,
                        res_model: 'crm.lead',
                        res_id: self.record.data.id,
                        mimetype: file.type
                    }],
                }).then(function (attachmentId) {
                    attachmentIds.push(attachmentId);

                    // Jika semua file sudah terupload, lanjutkan ke _markActivityDone
                    if (attachmentIds.length === fileCount) {
                        console.log("All files uploaded, marking activity as done.");
                        self._markActivityDone({
                    ...
                    self.do_warn("Error", "Failed to upload attachment: " + file.name);
                });
            };
            reader.onerror = function (err) {
                console.error("Error reading file:", err);
                self.do_warn("Error", "Failed to read file: " + file.name);
                };
            });
        });
