...
class RFQExportController(http.Controller):
    ...
    def export_selected_rfqs(self, **post):
        ...
        rfq_headers = [
            ...
            'Product', 'Quantity', 'UoM', 'Unit Price', 'Taxes', 'Analytic Tags'
        ]
        ...
        for rfq in rfqs:
            ...
            for line in lines:
                ...
                if line:
                    ...
                    rfq_sheet.write(row, 16, line.product_qty or 0.0, number_style)
                    rfq_sheet.write(row, 17, line.product_uom.name if line.product_uom else '', data_style)
                    rfq_sheet.write(row, 18, line.price_unit or 0.0, number_style)
                    taxes = ', '.join([tax.name for tax in line.taxes_id]) if line.taxes_id else ''
                    rfq_sheet.write(row, 19, taxes, data_style)
                    analytic_tags = ', '.join([tag.name for tag in line.analytic_tag_ids]) if hasattr(line, 'analytic_tag_ids') and line.analytic_tag_ids else ''
